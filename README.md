...
| Функция | Описание |
|---------|---------|
| **IPSet режимы** | Переключение между режимами фильтрации |
| **GameFilter** | Включение/отключение обработки игровых портов |
| **Обновление списков** | Загрузка актуальных списков доменов и IP |
| **Добавление доменов** | Быстрое добавление новых доменов в фильтры |
...
Для проверки работоспособности конфигураций откройте меню управления:
...
Выберите пункт **"5. Запустить тесты конфигураций"** и следуйте подсказкам.

**Возможности тестера:**

- **Стандартные тесты** — проверка доступности целевых сайтов (HTTP, TLS 1.2, TLS 1.3)
- **DPI тесты** — проверка обхода DPI блокировок на различных провайдерах
- **Выбор конфигов** — тестирование всех конфигов или выбранных вручную
- **Логирование** — сохранение результатов в файл для анализа

Тестер предложит выбрать:
1. Тип теста (стандартный или DPI)
2. Режим конфигов (все или выбранные)
3. Конкретные конфиги для тестирования (если выбран режим выбора)

Результаты сохраняются в `$HOME/zapret-configs/utils/log/`

### 🔄 Обновление репозитория

Для обновления локальной копии репозитория используйте:
...
> 
> Если ваша текущая конфигурация работает **идеально** и вы **не уверены** в совместимости новых изменений, **не обновляйтесь**!
> 
> Обновления могут содержать:
> - Изменения в конфигурациях, которые могут не работать на вашем провайдере
> - Новые требования к зависимостям
> - Изменения в поведении фильтрации
> 
> **Рекомендация:** Обновляйтесь только если:
> - ✅ Вы хотите попробовать новые конфигурации
> - ✅ Текущая конфигурация перестала работать
> - ✅ Вы готовы к возможным проблемам и можете откатиться назад
...
2. **Зависимости** — установите curl вручную через пакетный менеджер
3. **Бесконечное "подключение" к голосовому чату Discord** — запустите меню управления:
   ```bash
   $HOME/zapret-configs/utils-zapret.sh
   ```
   Выберите пункт **"4. Обновить файл hosts"** и следуйте подсказкам. Это обновит файл `/etc/hosts` необходимыми записями для корректной работы голосовых каналов Discord.

4. **Обход не работает / перестал работать**
   - Проверьте, что адрес ресурса записан в списках доменов или IP
   - Убедитесь, что zapret запущен и работает корректно
   - Попробуйте альтернативные конфигурации (например, `general(ALT)`, `general (FAKE_TLS_AUTO)` и т.д.)
   - Обновите списки доменов и IP через меню управления:
     ```bash
     $HOME/zapret-configs/utils-zapret.sh
     ```
     Выберите пункт **"3. Обновить список IPSet"**
   - Проверьте логи zapret для выявления ошибок
   - Попробуйте полную переустановку, если ничего не помогает
