...
- [`hostlists/list-general.txt`](./hostlists/list-general.txt) — для доменов (поддомены автоматически учитываются)
- [`hostlists/list-exclude.txt`](./hostlists/list-exclude.txt) — для исключения доменов (если IP сети указан в `ipset-all.txt`, но конкретный домен не надо фильтровать)
- [`hostlists/ipset-all.txt`](./hostlists/ipset-all.txt) — для IP адресов и подсетей
- [`hostlists/ipset-exclude.txt`](./hostlists/ipset-exclude.txt) — для исключения IP адресов и подсетей
...
**GameFilter:**
- Включен — обрабатывает игровые порты 1024-65535
- Выключен — обрабатывает только порт 12
