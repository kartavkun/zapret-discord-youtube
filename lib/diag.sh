...
# shellcheck source=incident.sh
# shellcheck source=strategy.sh
# shellcheck source=nfqws.sh
#
...
# Русское склонение для "N дней назад": иначе в шапке менеджера получается
# "обновлён 2 день(s) назад".
plural_days() {
  local n="$1"
  case "$n" in
    "" | *[!0-9]*)
      printf 'давность неизвестна\n'
      return
      ;;
  esac
  [ "$n" -eq 0 ] && {
  ...
  local tail100=$((n % 100)) tail10=$((n % 10))
  if [ "$tail100" -ge 11 ] && [ "$tail100" -le 14 ]; then
    printf '%s дней назад\n' "$n"
  elif [ "$tail10" -eq 1 ]; then
    printf '%s день назад\n' "$n"
  elif [ "$tail10" -ge 2 ] && [ "$tail10" -le 4 ]; then
    printf '%s дня назад\n' "$n"
  else
    printf '%s дней назад\n' "$n"
  fi
...
diag_bundle() {
  ...
  # но под set -e он оборвал бы сбор на первой же такой команде.
  {
    ...
    if [ -n "$latest" ]; then
      printf 'каталог: %s\n\n' "$latest"
      local f
      for f in summary.txt processes.txt lists.txt nfqueue.txt conntrack.txt; do
        [ -r "$latest/$f" ] || continue
        printf '== %s ==\n' "$f"
        cat "$latest/$f" || true
        printf '\n'
    ...
    printf '\n%s\n' '--- последний слепок планового перезапуска ---'
    local timer_snap
    timer_snap=$(find "${ZDY_AUTORESTART_SNAPSHOT_DIR:-/var/log/zapret-autorestart.d}" \
      -maxdepth 1 -name '*-timer.txt' -print 2>/dev/null | LC_ALL=C sort | tail -n 1)
    if [ -n "$timer_snap" ] && [ -r "$timer_snap" ]; then
      printf 'файл: %s\n\n' "$timer_snap"
      cat "$timer_snap" || true
    else
      printf 'нет: плановый перезапуск выключен или ещё не срабатывал\n'
    fi
