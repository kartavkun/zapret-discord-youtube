...
# Записи conntrack по портам, которыми пользуются Discord и QUIC.
#
# Именно это перезапуск службы стирает в первую очередь, поэтому снимать надо
# до него. Здесь же лежат адреса, к которым подключался пользователь, - из-за
# этого каталог снимков имеет права 700, а отчёт для issue - 600.
incident_collect_conntrack() {
  local src="$ZDY_PROC/net/nf_conntrack"

  if [ ! -r "$src" ]; then
    printf 'нет данных: %s недоступен (модуль nf_conntrack не загружен?)\n' "$src"
    return 0
  fi

  printf 'всего записей: %s\n\n' "$(grep -c . "$src" 2>/dev/null || printf '?')"
  grep -E 'dport=(443|19[23][0-9]{2}|50[0-9]{3})' "$src" 2>/dev/null | head -n 60
}
...
incident_snapshot() {
  ...
  incident_collect_conntrack >"$dir/conntrack.txt" 2>&1
