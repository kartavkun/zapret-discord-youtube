...
strategy_list() {
  ...
  for entry in "$dir"/*; do
    ...
    printf '%s\n' "$name"
  done | strategy_sort_natural

  eval "$saved_nullglob"
}

# Числа сортируются как числа: иначе general-alt-10 оказывается перед
# general-alt-2, и в меню это выглядит поломкой. sort -V не годится - его
# нет в части сборок busybox, поэтому дополняем числа нулями на время
# сортировки и снимаем дополнение обратно.
strategy_sort_natural() {
  # Строка разбирается слева направо с потреблением остатка. Замена "на
  # месте" здесь зацикливается: дополненное нулями число снова попадает под
  # /[0-9]+/ и дополняется бесконечно.
  awk '{
    key = ""
    rest = $0
    while (match(rest, /[0-9]+/)) {
      key = key substr(rest, 1, RSTART - 1) sprintf("%08d", substr(rest, RSTART, RLENGTH))
...
strategy_is_valid() {
  ...
  # shellcheck disable=SC2016  # тело для sh -c намеренно не разворачивается здесь
