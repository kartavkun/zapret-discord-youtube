...
assert_eq "general-alt-2" "$(strategy_resolve_name 'general-alt2' 2>/dev/null)" \
  "промежуточное имя резолвится"
...
nonascii=$(env LC_ALL=C grep -l '[^ -~]' "$REPO_DIR"/bin/* "$REPO_DIR"/lib/*.sh 2>/dev/null |
  while IFS= read -r f; do
    # кириллица разрешена, всё остальное вне ASCII - нет
    env LC_ALL=C.UTF-8 python3 - "$f" <<'PYEOF'
import sys
p = sys.argv[1]
for ch in open(p, encoding='utf-8').read():
    o = ord(ch)
    if o >= 128 and not (0x400 <= o <= 0x4FF):
        print(p)
        break
PYEOF
  done)
...
report=$(ZDY_ASSUME_YES=1 diag_bundle "$SANDBOX/report.txt" | tail -n 1)
